...
To explore whether Pokémon types are distinguishable based on their visual features, we look to apply clustering algorithms to their images. Since we are working with high-dimensional data (801x57600), we first apply dimension reduction techniques to mitigate the curse of dimensionality. Our primary tools were Principal Component Analysis (PCA) and Uniform Manifold Approximation and Projection (UMAP). In their own right, these methods also help us determine if type-based structure is embedded in high-dimensional image representations.

### Principal Component Analysis (PCA)

We began with PCA, a linear technique that identifies the directions of maximal variance in the dataset. After centering the flattened image matrix, we applied PCA and examined the variance

```{r echo=FALSE}
## Scree plot + Biplot
load("DimensionReduction/pca_scree.rds")
# load("DimensionReduction/pca_biplot.rds")

# print(pca_scree + pca_biplot)

# REPLACE WITH COMBINED IMAGE USING ggsave()
```

```{r echo=FALSE}
## Image compression
load("DimensionReduction/img_compress.rds")
print(sbs)
```
